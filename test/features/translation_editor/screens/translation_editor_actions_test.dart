...
void main() {
  ...
  group('TranslationProgressScreen', () {
    ...
    testWidgets('pause button pauses translation',
        (WidgetTester tester) async {
      final progressController = StreamController<
          Result<TranslationProgress, TranslationOrchestrationException>>();

      when(() => mockOrchestrator.pauseTranslation(
            batchId: any(named: 'batchId'),
      ...
      // Assert: Pause called
      verify(() => mockOrchestrator.pauseTranslation(batchId: 'test-batch'))
          .called(1);

      await progressController.close();
    });

    testWidgets('resume button resumes paused translation',
        (WidgetTester tester) async {
      final progressController = StreamController<
          Result<TranslationProgress, TranslationOrchestrationException>>();

      when(() => mockOrchestrator.resumeTranslation(
            batchId: any(named: 'batchId'),
          )).thenAnswer((_) async => const Ok(null));
      ...
      verify(() => mockOrchestrator.resumeTranslation(batchId: 'test-batch'))
          .called(1);
