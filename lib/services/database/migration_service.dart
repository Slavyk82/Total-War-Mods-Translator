...
/// Migration service for database schema initialization and updates.
///
/// This service orchestrates:
/// 1. Fresh database initialization from schema.sql
/// 2. Running incremental migrations for existing databases
...
class MigrationService {
  ...
  /// Ensure all incremental migrations are applied.
  ///
  /// This method can be called on any database (fresh or existing) to ensure
  /// that all registered migrations are applied. Each migration is idempotent
  ...
  static Future<List<MigrationResult>> ensurePerformanceIndexes() async {
    ...
    for (final migration in migrations) {
      ...
      try {
        ...
        // Continue with other migrations - they should be independent
      }
    }

    final applied = results.where((r) => r.success && !r.skipped).length;
