...
class RpfmServiceImpl implements IRpfmService {
  ...
  Future<Result<String, RpfmServiceException>> createPack({
    ...
    try {
      ...
      final schemaDir = await _cliManager.getSchemaPath();
      if (schemaDir == null || schemaDir.isEmpty) {
        return Err(const RpfmServiceException(
          'RPFM schema path not configured. Please set it in Settings > RPFM Tool.',
        ));
      ...
      _logger.info('Using schema: $schemaFile');

      // Step 1: Create empty pack
      // Syntax: rpfm_cli.exe --game <GAME> pack create --pack-path <PATH>
      ...
      _logger.info('Empty pack created, now adding TSV files with conversion...');

      // Step 2: Find and add TSV files with --tsv-to-binary conversion
      // TSV files are generated by LocFileServiceImpl in RPFM format
      final allFiles = await _listDirectoryFiles(inputDirectory);
      final tsvFiles =
          allFiles.where((f) => f.toLowerCase().endsWith('.tsv')).toList();

      _logger.info('Found ${tsvFiles.length} TSV files to add');

      if (tsvFiles.isEmpty) {
        // Fallback: check for .loc files (legacy support)
        final locFiles =
            allFiles.where((f) => f.toLowerCase().endsWith('.loc')).toList();
        if (locFiles.isNotEmpty) {
          _logger.warning(
              'No TSV files found, falling back to .loc files (may cause issues)');
          for (final filePath in locFiles) {
            ...
            if (result.exitCode != 0) {
              final error = RpfmOutputParser.parseErrorMessage(result.stderr);
              _logger.warning('Failed to add .loc file: $error');
            }
          }
        }
      } else {
        ...
        for (final tsvFilePath in tsvFiles) {
          ...
          // e.g., text/db/!!!!!!!!!!_FR_text.loc.tsv -> text/db/!!!!!!!!!!_FR_text.loc
          final targetPath = relativePath
              .replaceAll('.tsv', '')
              .replaceAll('\\', '/');

          _logger.info('Adding TSV: $relativePath -> $targetPath');

          // Syntax: rpfm_cli.exe --game <GAME> pack add --pack-path <PACK_PATH>
          //         --file-path <FILE_PATH;INTERNAL_PATH> --tsv-to-binary <SCHEMA_PATH>
          ...
          if (result.exitCode != 0) {
            final error = RpfmOutputParser.parseErrorMessage(result.stderr);
            _logger.error('Failed to add TSV file: $error');
            _logger.error('RPFM stderr: ${result.stderr}');
            _logger.error('RPFM stdout: ${result.stdout}');
            return Err(RpfmPackingException(
          ...
          _logger.info('Successfully added: $targetPath');
        }
      }
      ...
      _logger.info('Pack created successfully: $outputPackPath (${packSize ~/ 1024}KB)');
